...

{
    public partial class frmSelect_Outbound_Load : Form
    {
        ...
        public void Reset_Filter(bool UpdateLists=true )
        {
            ...
            if (UpdateLists)
            {
                this.nWDataset.EnforceConstraints = false;
                this.vw_Outbound_LoadTableAdapter.Fill(this.nWDataset.vw_Outbound_Load);
                this.outbound_Load_CarriersTableAdapter.Fill(this.nWDataset.Outbound_Load_Carriers, Settings.Location_Id);
                this.outbound_Load_CropsTableAdapter.Fill(this.nWDataset.Outbound_Load_Crops, Settings.Location_Id);
                this.outbound_Load_DestinationsTableAdapter.Fill(this.nWDataset.Outbound_Load_Destinations);
           
            }

            this.FirstDay = DateTime.Now;// new DateTime(DateTime.Now.Year, 1, 1);
        ...
        {
            Loading.Show("Creating New Outbound Load", Form.ActiveForm);
        ...
        public void UpdateData()
        {
            if (!Wait)
            {
                int Position = this.vwOutboundLoadBindingSource.Position; 
                string Crop = null;
                ...
                this.vw_Outbound_LoadTableAdapter.FillByFilter(nWDataset.vw_Outbound_Load,Crop ,Closed,  StartDate, EndDate,Destination,Carrier,Void, Settings.Location_Id);
                ...
                try
                {
                    this.vwOutboundLoadBindingSource.Position = Position;
                }
                catch
                {

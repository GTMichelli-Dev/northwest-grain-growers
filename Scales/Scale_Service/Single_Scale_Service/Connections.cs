                ...
                if (!Initialized)
                {
                    try
                    {
...

                        {
                            ...
                            LocalWebService.LocalDataSet.Weigh_ScalesDataTable allWeigh_ScalesDataTable = new LocalWebService.LocalDataSet.Weigh_ScalesDataTable();

                            using (LocalWebService.LocalWebService Proxy = new LocalWebService.LocalWebService())
                            {


                                allWeigh_ScalesDataTable = Proxy.GetScales();
                            }
                            string scaleDescription = Properties.Settings.Default.ScaleDescription ?? "";
                            if (scaleDescription == "")
                            {
                                Eventlog.WriteEntry($"Target Scale Not Defind", EventLogEntryType.Error);
                            }
                            else
                            {
                                Eventlog.WriteEntry($"Targeting Scale <{scaleDescription}>", EventLogEntryType.Information);
                                foreach (LocalWebService.LocalDataSet.Weigh_ScalesRow row in allWeigh_ScalesDataTable)
                                    if (row.Description.ToUpper() == scaleDescription.ToUpper())
                                    {
                                        weigh_ScalesDataTable.ImportRow(row);
                                    }

                                foreach (LocalWebService.LocalDataSet.Weigh_ScalesRow row in weigh_ScalesDataTable)
                                {

                                    Eventlog.WriteEntry($"Scale {scaleDescription} Located @{row.IP_Address}  ", EventLogEntryType.Information);

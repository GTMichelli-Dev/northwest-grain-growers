    ...
    public class Connections : IDisposable
    {
        ...
        public void GetScales()
        {
            ...
            do
            {
                ...
                //if (weigh_ScalesDataTable== null )
                //using (LocalWebService.LocalWebService Proxy = new LocalWebService.LocalWebService())
                //{
                //    LocalWebService.LocalDataSet.Weigh_ScalesDataTable CurrentScales = Proxy.GetScales();
                //    foreach (LocalWebService.LocalDataSet.Weigh_ScalesRow Newrow in CurrentScales )
                //    {
                //        foreach(LocalWebService.LocalDataSet.Weigh_ScalesRow Oldrow in weigh_ScalesDataTable)
                //        {
                ...
                if (!Initialized || weigh_ScalesDataTable.Count==0)
                {
                    try
                    {
                        Remove_Scales();

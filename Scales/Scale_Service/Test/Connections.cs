                ...
                if (!Initialized )
                {
                    try
                    {
...

                        {
                            ...
                            string Scale1 = ConfigurationManager.AppSettings["Scale_1_Address"];
                            string Scale2 = ConfigurationManager.AppSettings["Scale_2_Address"];

                            foreach (LocalWebService.LocalDataSet.Weigh_ScalesRow row in weigh_ScalesDataTable)
                            {
                                if (row.Scale_Type == "SMA" && row.IP_Address== Scale1 || row.Scale_Type == "SMA" && row.IP_Address == Scale2)
                                {
                                    Add_SMA_Scale(row.Description, row.IP_Address, row.Port, 700, row.Location_Id);
                                }
                                else if ((row.Scale_Type == "IDS" && row.IP_Address == Scale1) || (row.Scale_Type == "IDS" && row.IP_Address == Scale2))
                                {

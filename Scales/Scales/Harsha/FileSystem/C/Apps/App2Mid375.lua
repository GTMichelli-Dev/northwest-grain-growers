...
--## DESC=This is the Mid375 Checkmate App (App2Mid375.lua)

]]
require("ReqDebug")AppName="MID375"SerialNumber=awtx.setupMenu.getSerialNumber()AWTX_LUA_UNDEFINED=0
AWTX_LUA_WEIGHT=1
AWTX_LUA_STRING=2
AWTX_LUA_DATE=3
AWTX_LUA_TIME=4
...
SITE_CHINA=4
SITE_INDIA=5
function calcAfterUnitsChange()awtx.weight.graphEnable(1,0)system=awtx.hardware.getSystem(1)config=awtx.weight.getConfig(1)wt=awtx.weight.getCurrent(1)setpoint.setOutputValues()checkweigh.updateWeightBasedSettingsAfterUnitsChange()if checkweigh.GraphFlag~=0 then
...
require("ReqScaleKeys")require("ReqStats")require("ReqTare")require("ReqSetpoint")require("ReqCheckWeigh")require("ReqBattery")require("ReqAppMenu")function awtx.keypad.KEY_SELECT_UP()if selectHoldFlag<HowManyRepeatsMakeAHold then
...
displayPRNTOT()wt=awtx.weight.getLastPrint(1)awtx.printer.PrintFmt(checkweigh.TotalFmt)end
...
checkweigh.sampleTarget()else
checkweigh.enterTargetWithIncrement()end
end
targetHoldFlag=0
end
...
checkweigh.initAll()ID.initIDPrintTokens()tare.initTarePrintTokens()setpoint.initSetpointPrintTokens()checkweigh.initCheckPrintTokens()transaction.initTransPrintTokens()Stat.initStatsPrintTokens()SMAPC.initPrintTokens()usermode=awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)printAppDebug("\n Startup App2Mid375.lua non-event ")while true do

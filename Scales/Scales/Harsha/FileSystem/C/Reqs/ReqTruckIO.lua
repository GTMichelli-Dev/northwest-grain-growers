...
APstr="A"if tmpOSDateTime.hour>12 then
tmpOSDateTime.hour=tmpOSDateTime.hour-12
APstr="P"end
...
errorState1,dbWtUnit=awtx.weight.unitStrToUnitIndex(dbunits)errorState2,newWtUnit=awtx.weight.unitStrToUnitIndex(newunits)if errorState1==0 and errorState2==0 then
newtruckId=row[1]newtareFleet=awtx.weight.convertWeight(dbWtUnit,row[2],newWtUnit,1)newweight1=awtx.weight.convertWeight(dbWtUnit,row[3],newWtUnit,1)newweight1date=row[4]newweight1time=row[5]newgrossTotal=awtx.weight.convertWeight(dbWtUnit,row[6],newWtUnit,1)newnetTotal=awtx.weight.convertWeight(dbWtUnit,row[7],newWtUnit,1)newtareTotal=awtx.weight.convertWeight(dbWtUnit,row[8],newWtUnit,1)newtransCount=row[9]else
...
errorState1,dbWtUnit=awtx.weight.unitStrToUnitIndex(dbunits)errorState2,newWtUnit=awtx.weight.unitStrToUnitIndex(newunits)if errorState1==0 and errorState2==0 then
newtareFleet=awtx.weight.convertWeight(newWtUnit,newtareFleet,dbWtUnit,1)else
newtareFleet=newtareFleet
end
end
function truck.importGTN(label)local errorCode,currentRPN
currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)awtx.display.writeLine("Busy")errorCode=awtx.os.convertCsvFileToTable_LUA(DB_FileLocation_AppData,"tblGTN",[[G:\GTN_CSV\gtn.csv]])if errorCode==0 then
...
currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)awtx.display.writeLine("Busy")errorCode=awtx.os.convertTableToCsvFile_LUA(DB_FileLocation_AppData,"tblGTN","gtn.csv",[[G:\GTN_CSV]])if errorCode==0 then
awtx.display.writeLine("Done")else
awtx.display.writeLine(string.format("Fail=%d",errorCode))end
awtx.os.sleep(1000)awtx.keypad.set_RPN_mode(currentRPN)end

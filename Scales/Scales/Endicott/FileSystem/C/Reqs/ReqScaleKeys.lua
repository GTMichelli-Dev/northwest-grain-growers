...
if FirmwareVersionNum>=002000001002 then
newfirmware=true
else
newfirmware=false
end
...
usermodeSMAEntry=0
function SMAPC.setuserIDSTR(newuserIDSTR)SMAPC.userIDSTR=newuserIDSTR
SMAPC.setPrintTokens()local currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)usermodeSMAEntry=awtx.display.setMode(DISPLAY_MODE_USER_SCRIPT)awtx.display.writeLine("LOGON")SMAPC.SMAmode=SMALogonMode
SMAPC.setPrintTokens()waitingForKeyState=1
awtx.keypad.useAlternateLuaKeyboardEvents()end
function SMAPC.finish_setuserIDSTR()local idmaxVal,isEnterKey
local newuserIDSTR=SMAPC.userIDSTR
idmaxVal=8
newuserIDSTR,isEnterKey=awtx.keypad.enterString(newuserIDSTR,idmaxVal,entertime)awtx.display.setMode(usermodeSMAEntry)if isEnterKey then
...
function SMAPC.setuserIDNUM(newuserIDNUM)SMAPC.userIDNUM=newuserIDNUM
SMAPC.setPrintTokens()local currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)usermodeSMAEntry=awtx.display.setMode(DISPLAY_MODE_USER_SCRIPT)awtx.display.writeLine("LOGON")SMAPC.SMAmode=SMALogonMode
SMAPC.setPrintTokens()waitingForKeyState=2
awtx.keypad.useAlternateLuaKeyboardEvents()end
function SMAPC.finish_setuserIDNUM()local idminVal,idmaxVal,isEnterKey
local newuserIDNUM=SMAPC.userIDNUM
idminVal=0
idmaxVal=9999999
newuserIDNUM,isEnterKey=awtx.keypad.enterInteger(newuserIDNUM,idminVal,idmaxVal,entertime)awtx.display.setMode(usermodeSMAEntry)if isEnterKey then
SMAPC.userIDNUM=newuserIDNUM
SMAPC.setPrintTokens()else
end
SMAPC.SMAmode=SMAWeighMode
SMAPC.setPrintTokens()awtx.keypad.set_RPN_mode(currentRPN)end
function SMAPC.setMessage(newMessage)if(newMessage=="")then
clearMessage()else
SMAPC.message=newMessage
...
function displayMessage(message,time)awtx.keypad.set_RPN_mode(0)awtx.display.setMode(DISPLAY_MODE_USER_SCRIPT)awtx.display.writeLine(message)if time==0 then
waitingForKeyState=3
awtx.keypad.useAlternateLuaKeyboardEvents()else
awtx.os.sleep(time*1000)awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)awtx.keypad.set_RPN_mode(1)end
end
function finish_displayMessage()awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)awtx.keypad.set_RPN_mode(1)end
function clearMessage()awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)awtx.keypad.set_RPN_mode(1)awtx.keypad.useStandardLuaKeyboardEvents()end
waitingForKeyState=0
function WAITING_FOR_KEY_EVENT(keycode)awtx.keypad.useStandardLuaKeyboardEvents()if waitingForKeyState==1 then
SMAPC.finish_setuserIDSTR()elseif waitingForKeyState==2 then
SMAPC.finish_setuserIDNUM()elseif waitingForKeyState==3 then
finish_displayMessage()end
end
awtx.keypad.registerAlternateKeyEvent(WAITING_FOR_KEY_EVENT)SMAPC.SMAOnlineRPN=awtx.keypad.get_RPN_mode()SMAPC.SMAOnlineUsermode=awtx.display.getMode()function SMAOnlineEvent(flag)if(flag==1)then
SMAPC.SMAmode=SMAWeighMode
...
awtx.serial.registerSMAOnlineEvent(SMAOnlineEvent)else
SMAPC={}SMAWeighMode=0
SMACheckMode=1
SMALogonMode=2
SMAFindMeMode=3
...
awtx.serial.registerSMAOnlineEvent(SMAOnlineEvent)end
function displayWORD(word)local usermode,currentRPN
...
DB_FileLocation_AppConfig=[[C:\Database\AppConfig.db]]dbFile=sqlite3.open(DB_FileLocation_AppConfig)result=dbFile:exec("CREATE TABLE IF NOT EXISTS tblScannerConfig (varID TEXT PRIMARY KEY, value TEXT)")dbFile:close()end

...
usermode=awtx.display.setMode(DISPLAY_MODE_USER_SCRIPT)currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)DB_FileLocation_Dummy=[[C:\Database\Dummy.db]]dbFile=sqlite3.open(DB_FileLocation_Dummy)awtx.display.setMode(usermode)awtx.keypad.set_RPN_mode(currentRPN)end
function file.deleteDummyDatabase()local simAppPath,usermode,currentRPN
usermode=awtx.display.setMode(DISPLAY_MODE_USER_SCRIPT)currentRPN=awtx.keypad.get_RPN_mode()awtx.keypad.set_RPN_mode(0)DB_FileLocation_Dummy=[[C:\Database\Dummy.db]]result=awtx.os.deleteFile(DB_FileLocation_Dummy)awtx.display.setMode(usermode)awtx.keypad.set_RPN_mode(currentRPN)end

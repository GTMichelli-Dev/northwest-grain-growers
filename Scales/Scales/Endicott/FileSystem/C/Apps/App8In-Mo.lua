...
--## DESC=This is the In Motion Weighing App (App8In-Mo.lua)

]]
require("ReqDebug")AppName="IN-MO"SerialNumber=awtx.setupMenu.getSerialNumber()AWTX_LUA_UNDEFINED=0
AWTX_LUA_WEIGHT=1
AWTX_LUA_STRING=2
AWTX_LUA_DATE=3
AWTX_LUA_TIME=4
...
require("ReqTare")require("ReqSetpoint")require("ReqIn-Mo")require("ReqBattery")require("ReqAppMenu")lblF1="  ID"lblF2=""lblF3="UNITS"lblF4="SCALE"lblF5=" DISP"function awtx.keypad.KEY_F1_UP()if f1HoldFlag<HowManyRepeatsMakeAHold then
enterId()end
f1HoldFlag=0
end
...
function awtx.keypad.KEY_SELECT_DOWN()local usermode
...
ID.initIDPrintTokens()tare.initTarePrintTokens()setpoint.initSetpointPrintTokens()inmo.initInMoPrintTokens()inmo.initXR4500PrintToken()SMAPC.initPrintTokens()usermode=awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)printAppDebug("\n Startup App8In-Mo.lua non-event ")while true do
if inmo.delayFlag then
inmo.timetick=inmo.timetick+1
if inmo.timetick>inmo.delaytick then
awtx.weight.clearInmotion()inmo.delayFlag=false
inmo.timetick=0
if inmo.Switch2Gross==1 then
if wt.tare>0 then
awtx.weight.setActiveValue(VAL_NET)else
awtx.weight.setActiveValue(VAL_GROSS)end
end
end
end

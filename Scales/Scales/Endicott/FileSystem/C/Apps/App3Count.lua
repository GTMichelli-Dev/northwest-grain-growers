...
--## DESC=This is the Parts Counting App (App3Count.lua)

]]
require("ReqDebug")AppName="COUNT"SerialNumber=awtx.setupMenu.getSerialNumber()AWTX_LUA_UNDEFINED=0
AWTX_LUA_WEIGHT=1
AWTX_LUA_STRING=2
AWTX_LUA_DATE=3
AWTX_LUA_TIME=4
...
require("ReqScaleKeys")require("ReqTare")require("ReqSetpoint")require("ReqCount")require("ReqBattery")require("ReqAppMenu")function TARECOMPLETEEVENT(eventResult,eventResultString)if eventResult~=0 then
displayCANT()else
if count.SampleStepFlag==1 then
usermode=awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)count.checkSample()end
end
end
awtx.weight.registerTareCompleteEvent(TARECOMPLETEEVENT)function ZEROCOMPLETEEVENT(eventResult,eventResultString)local usermode
...
if count.PrintTotalFlag==1 then
displayPRNTOT()wt=awtx.weight.getCurrent(1)awtx.printer.PrintFmt(count.TotalFmt)end
if count.ClearTotalFlag==1 then
count.newClrAccum()end
end
end
...
enterId()counttable[newCountChannel].accId=ID.id
count.countDBStore()end
end
function awtx.keypad.KEY_F1_UP()if f1HoldFlag<HowManyRepeatsMakeAHold then
count.checkSample()end
f1HoldFlag=0
end
function awtx.keypad.KEY_SAMPLE_DOWN()usermode=awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)sampleHoldFlag=0
end
...
ID.initIDPrintTokens()tare.initTarePrintTokens()setpoint.initSetpointPrintTokens()count.initCountPrintTokens()SMAPC.initPrintTokens()usermode=awtx.display.setMode(DISPLAY_MODE_SCALE_OBJECT)printAppDebug("\n Startup App3Count.lua non-event ")while true do

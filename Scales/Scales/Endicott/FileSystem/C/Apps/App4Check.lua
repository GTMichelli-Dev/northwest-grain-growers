...
--## DESC=This is the Simple Checkweighing App (App4Check.lua)

]]
require("ReqDebug")AppName="CHECK"SerialNumber=awtx.setupMenu.getSerialNumber()AWTX_LUA_UNDEFINED=0
AWTX_LUA_WEIGHT=1
AWTX_LUA_STRING=2
AWTX_LUA_DATE=3
AWTX_LUA_TIME=4
...
SITE_CHINA=4
SITE_INDIA=5
function calcAfterUnitsChange()system=awtx.hardware.getSystem(1)config=awtx.weight.getConfig(1)wt=awtx.weight.getCurrent(1)setpoint.setOutputValues()checkweigh.updateWeightBasedSettingsAfterUnitsChange()end
...
require("ReqScaleKeys")require("ReqStats")require("ReqTare")require("ReqSetpoint")require("ReqCheckWeigh")require("ReqBattery")require("ReqAppMenu")function awtx.keypad.KEY_F1_UP()if f1HoldFlag<HowManyRepeatsMakeAHold then
wt=awtx.weight.getCurrent(1)if not wt.inGrossBand then
checkweigh.sampleTarget()else
result=checkweigh.enterTargLo()if result then
checkweigh.enterTargHi()end
end
end
f1HoldFlag=0
end

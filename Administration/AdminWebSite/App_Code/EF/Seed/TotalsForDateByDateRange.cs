...
public class TotalsForDateByDateRange
{
    ...
    public static List<TotalsForDateByDateRange> GetTotalsForDatesByDate(int? locationId, DateTime startDate, DateTime endDate)
    {
        using (var context = new SeedModel())
        {
            ...
            var query = @"SELECT        Location_ID, Location, Date_Shipped, Total_Lbs, Total_Bushels, Total_Loads, Clean_Lbs, Clean_Bushels, Clean_Loads, Treated_Lbs, TreatedBushels, Treated_Loads
                FROM            dbo.TotalsForDateByDateRange(@StartDate, @EndDate, @Location_ID) AS VarietyTotalsForDateByDateRange_1
                ORDER BY Location_ID, Date_Shipped ";

            return context.Database.SqlQuery<TotalsForDateByDateRange>(query, sd, ed, lId).ToList();

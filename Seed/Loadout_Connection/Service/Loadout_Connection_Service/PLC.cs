...
namespace WW_LGX_PLC
{
...

    {
        ...
        public void StartScanning(EventLog ev)
        {
            eventLog1 = ev;
            if (!scanningTags)
            {
                eventLog1.WriteEntry(string.Format("Starting Scan Thread"), EventLogEntryType.Information );
        ...
        private void ScanTags()
        {
            ...
            using (var client = new Libplctag())
            {
                ...
                {
                    eventLog1.WriteEntry("Scan Tag Error " + ex.ToString(), EventLogEntryType.Warning);
                }
                finally
                {

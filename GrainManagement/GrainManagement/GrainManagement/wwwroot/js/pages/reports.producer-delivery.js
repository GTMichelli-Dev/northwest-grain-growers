// Producer Delivery Report. Same filter shape as the Producer
// Commodity report — multi-select location + crop, optional producer
// search modal — but with a Group By toggle (primary account vs split
// group) that drives whether the Split Group columns are shown.
(function () {
    'use strict';
    ...
    function fetchData() {
        ...
        fetch('/api/ReportBuilder/ProducerDelivery', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
    ...
    function exportToExcel() {
        ...
        var worksheet = workbook.addWorksheet('Producer Delivery');
        DevExpress.excelExporter.exportDataGrid({
            ...
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }),
                'ProducerDelivery_' + fromVal + '_to_' + toVal + '.xlsx');
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Toggle re-renders the column visibility immediately so the
        // operator sees the shape change without re-applying. Data
        // doesn't refresh until they click Apply, since the toggle
        // also changes the server-side grouping.

// Producer Commodity Report By District. Intake-only, multi-select
// locations + crops, optional producer / split group search via the
// modal popup. Variety columns hidden by default and revealed only
// when at least one row in the result set is a seed item.
(function () {
    'use strict';
        ...
        return fetch('/api/ReportBuilder/Locations?districtId=' + id)
            ...
            .then(function (list) {
                ...
                if ($locTags) {
                    ...
                    // Default to "all" — the user said default to all, so
                    // we pre-select every item the picker offers.
    ...
    function fmtInt(v) {
        var n = Number(v); return isFinite(n) ? Math.round(n).toLocaleString() : '';
    }
    ...
    function buildGrid() {
        $grid = $('#rb-grid').dxDataGrid({
            dataSource: [],
            keyExpr: ['LocationId', 'PrimaryAccountId', 'SplitGroupId', 'CropId', 'VarietyId'],
            showBorders: true,
            showRowLines: true,
            ...
            columns: [
                ...
                { dataField: 'SplitGroupId', caption: 'Split #', alignment: 'right', width: 100 },
                { dataField: 'SplitGroupDescription', caption: 'Split Description' },
                { dataField: 'Crop', caption: 'Crop' },
                { dataField: 'CropId', caption: 'Crop Id', alignment: 'right', width: 90, visible: false },
                { dataField: 'Variety', caption: 'Variety' },
                { dataField: 'VarietyId', caption: 'Variety Id', alignment: 'right', width: 100 },
            ...
            summary: {
                ...
                totalItems: [
                    ...
                    { column: 'NetUnits', summaryType: 'sum',
                      valueFormat: function (v) { return fmtNum(v, 2); }, displayFormat: 'Total: {0}' },
                    { column: 'LoadCount', summaryType: 'sum',
    ...
    function fetchData() {
        ...
        fetch('/api/ReportBuilder/ProducerCommodity', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
            ...
            rows = rows || [];
            // Hide the Variety column block if no row is a seed crop —
            // keeps the grid tidy for normal commodity reports.
            var anySeed = rows.some(function (r) { return !!r.IsSeed; });
            $grid.columnOption('Variety',   'visible', anySeed);
            $grid.columnOption('VarietyId', 'visible', anySeed);
    ...
    function exportToExcel() {
        ...
        var worksheet = workbook.addWorksheet('Producer Commodity');
        DevExpress.excelExporter.exportDataGrid({
            ...
            saveAs(new Blob([buffer], { type: 'application/octet-stream' }),
                'ProducerCommodity_' + fromVal + '_to_' + toVal + '.xlsx');
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Producer popup wiring.
        var producerModal = new bootstrap.Modal(document.getElementById('rb-producer-modal'));
        document.getElementById('rb-pick-producer').addEventListener('click', function () {
            document.getElementById('rb-producer-input').value = producerSearch || '';
            producerModal.show();
        });
        document.getElementById('rb-producer-apply').addEventListener('click', function () {
            producerSearch = (document.getElementById('rb-producer-input').value || '').trim();
            document.getElementById('rb-producer-summary').textContent =
                producerSearch ? ('Producer: ' + producerSearch) : '';
        });
        document.getElementById('rb-producer-clear').addEventListener('click', function () {
            producerSearch = '';
            document.getElementById('rb-producer-input').value = '';
            document.getElementById('rb-producer-summary').textContent = '';
        });

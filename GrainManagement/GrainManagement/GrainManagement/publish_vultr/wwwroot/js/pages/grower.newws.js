(function () {
    ...
    var SEL = {
        ...
        growerHauling:   '#nwsGrowerHauling',
        customHauler:    '#nwsCustomHauler',
        haulerDetails:   '#nwsHaulerDetails',
        hauler:          '#nwsHauler',
        rateType:        '#nwsRateType',
        milesGroup:      '#nwsMilesGroup',
        miles:           '#nwsMiles',
        calcRateGroup:   '#nwsCalcRateGroup',
        calcRate:        '#nwsCalcRate',
        customDescGroup: '#nwsCustomDescGroup',
        customRateDesc:  '#nwsCustomRateDesc',
        customRateGroup: '#nwsCustomRateGroup',
    ...
    var _haulerMode  = null;         // 'grower' | 'custom'
    ...
    function showStep2() {
        ...
        // Reset hauler state
        _haulerMode = null;
        $(SEL.haulerDetails).prop('hidden', true);
    ...
    function showStep3(wsId) {
        ...
        $(SEL.successInfo).html(
            '<strong>Weight Sheet #' + wsId + '</strong> has been created.' +
    ...
    function initHaulerStep() {
        ...
        // Rate type dropdown
        $(SEL.rateType).dxSelectBox({
            dataSource: [
                { Value: 'A', Text: 'Along the field (A)' },
                { Value: 'C', Text: 'Custom (C)' },
                { Value: 'F', Text: 'Farm storage (F)' },
            ],
            valueExpr:   'Value',
            displayExpr: 'Text',
            ...
            onValueChanged: function (e) {
                var val = e.value;
                // A or F: show miles + calculated rate
                var showMiles  = (val === 'A' || val === 'F');
                var showCustom = (val === 'C');

                $(SEL.milesGroup).prop('hidden', !showMiles);
                $(SEL.calcRateGroup).prop('hidden', !showMiles);
                $(SEL.customDescGroup).prop('hidden', !showCustom);
                $(SEL.customRateGroup).prop('hidden', !showCustom);
        ...
        // Miles input — value starts as undefined so we can detect untouched state
        $(SEL.miles).dxNumberBox({
            ...
            onValueChanged: async function (e) {
                ...
                if (!_milesEntered) { $(SEL.calcRate).val(''); return; }
                var rateTypeInst = dxInstance(SEL.rateType);
                var rateTypeVal = rateTypeInst ? rateTypeInst.option('value') : null;
                if (!rateTypeVal || rateTypeVal === 'C') return;

                try {
                    var data = await $.getJSON('/api/Lookups/HaulerRateForMiles?rateType=' + rateTypeVal + '&miles=' + e.value);
        ...
        // Grower Hauling → skip to create weight sheet
        $(SEL.growerHauling).on('click', function () {
            _haulerMode = 'grower';
            createWeightSheet();
        });

        // Custom Hauler → show hauler details
        $(SEL.customHauler).on('click', async function () {
            _haulerMode = 'custom';
            $(SEL.haulerDetails).prop('hidden', false);

            // Load haulers list
            try {
                var haulers = await $.getJSON('/api/Lookups/Haulers');
                var haulerInst = dxInstance(SEL.hauler);
                if (haulerInst) haulerInst.option('dataSource', haulers);
            } catch (ex) {
                showAlert('Failed to load haulers.', 'danger');
            }
        });

        // Create Weight Sheet from hauler step
        $(SEL.createWsBtn).on('click', function () {
            createWeightSheet();
        });
    }

    function resetHaulerFields() {
        var haulerInst   = dxInstance(SEL.hauler);
        var rateTypeInst = dxInstance(SEL.rateType);
        var milesInst    = dxNumberInstance(SEL.miles);
        var crInst       = dxNumberInstance(SEL.customRate);
        ...
        if (rateTypeInst) rateTypeInst.reset();
        ...
        $(SEL.milesGroup).prop('hidden', true);
        $(SEL.calcRateGroup).prop('hidden', true);
        $(SEL.customDescGroup).prop('hidden', true);
        $(SEL.customRateGroup).prop('hidden', true);
    }
    ...
    async function createWeightSheet() {
        ...
        if (_haulerMode === 'custom') {
            // Validate custom hauler fields
            var haulerInst   = dxInstance(SEL.hauler);
            var rateTypeInst = dxInstance(SEL.rateType);

            var haulerId  = haulerInst ? haulerInst.option('value') : null;
            var rateType  = rateTypeInst ? rateTypeInst.option('value') : null;
            ...
            if (!rateType) {
                $(SEL.haulerError).text('Please select a rate type.').prop('hidden', false);
                return;
            }

            payload.HaulerId = haulerId;

            if (rateType === 'A' || rateType === 'F') {
                var milesInst = dxNumberInstance(SEL.miles);
                ...
                // Look up the rate
                try {
                    var rateData = await $.getJSON('/api/Lookups/HaulerRateForMiles?rateType=' + rateType + '&miles=' + miles);
        ...
        var btn = _haulerMode === 'custom' ? $(SEL.createWsBtn) : $(SEL.growerHauling);
